...
export class App {
  ...
  private eventsWhitelist: Set<Events>;
  ...
  constructor(
    ...
    this.env = env;
    this.eventsWhitelist = new Set(events);
  ...
  public recordEvents(data: Record<string, any>[]) {
    return this.networkController.recordEvents(data);
  }

  public collectEvent(event_name: Events, requestBody?: Record<string, any>) {
    // Only collect whitelisted events
    if (!this.eventsWhitelist.has(event_name)) {
      return;
    }
